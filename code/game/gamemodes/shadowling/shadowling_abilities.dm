...
/obj/effect/proc_holder/spell/targeted/shadowling_hivemind/cast(list/targets)
	for(var/mob/living/user in targets)
		var/text = sanitize((user, "What do you want to say to fellow thralls and shadowlings?.", "Hive Chat", ""))
		if(!text)
			return
...
/obj/effect/proc_holder/spell/targeted/shadowlingAscendantTransmit/cast(list/targets)
	for(var/mob/living/user in targets)
		var/text = sanitize((user, "What do you want to say to everything on and near [world.name]?.", "Transmit to World", ""))
		if(!text)
			return
