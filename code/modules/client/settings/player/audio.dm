...
/datum/pref/player/audio
	...
	value_variations = list(0, 100)

	var/volume_channel

/datum/pref/player/audio/sanitize_value(new_value)
	return sanitize_integer(new_value, 0, 100, initial(value))

/datum/pref/player/audio/on_update(client/parent, old_value)
	parent?.mob?.playsound_local(null, 'sound/weapons/saberon.ogg', volume_channel, vary = FALSE, channel = CHANNEL_VOLUMETEST)
...
/datum/pref/player/audio/lobby/on_update(client/parent, old_value)
	parent?.mob?.playsound_music_update_volume(VOL_MUSIC, CHANNEL_MUSIC)
	..()
...
/datum/pref/player/audio/ambient/on_update(client/parent, old_value)
	parent?.mob?.playsound_music_update_volume(VOL_AMBIENT, CHANNEL_AMBIENT)
	parent?.mob?.playsound_music_update_volume(VOL_AMBIENT, CHANNEL_AMBIENT_LOOP)
	..()

// effect_master
/datum/pref/player/audio/effect_master
	name = "Effects: Master"
	description = "Общая громкость игровых эффектов"
	value = 100
...
// effect_misc
/datum/pref/player/audio/effect_misc
...
/datum/pref/player/audio/admin_sound/on_update(client/parent, old_value)
	parent?.mob?.playsound_music_update_volume(VOL_ADMIN, CHANNEL_ADMIN)
	..()
...
/datum/pref/player/audio/jukebox/on_update(client/parent, old_value) // todo
	var/datum/media_manager/media = parent.?media
	..()
